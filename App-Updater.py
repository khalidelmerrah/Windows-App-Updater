...
class WingetUpdaterUI:
    ...
    def show_about(self):
        ...
        ctk.CTkLabel(frame, text="Windows App Updater", font=("Segoe UI", 18, "bold")).pack(pady=(10, 2))
        ctk.CTkLabel(frame, text=APP_VERSION_ONLY, font=("Segoe UI", 12)).pack(pady=(0, 4))
        ctk.CTkLabel(frame, text="A modern GUI for batch-updating Windows apps\nusing winget package manager.",
                     font=("Segoe UI", 11), justify="center", wraplength=400).pack(pady=(0, 12))
        # Credits
        credits = ctk.CTkFrame(frame, fg_color="transparent")
        credits.pack(fill="x", padx=12, pady=(0, 8))
        ctk.CTkLabel(credits, text="Original author", font=("Segoe UI", 10), text_color="#888888").pack(anchor="w")
        ctk.CTkButton(credits, text="ilukezippo (BoYaqoub)", font=("Segoe UI", 11),
                       fg_color="transparent", hover_color="#2a2a2a", text_color="#4a9eff",
                       anchor="w", height=24, corner_radius=3,
                       command=lambda: webbrowser.open("https://github.com/ilukezippo/Windows-App-Updater")).pack(anchor="w")
        ctk.CTkLabel(credits, text="Enhanced by", font=("Segoe UI", 10), text_color="#888888").pack(anchor="w", pady=(6, 0))
        ctk.CTkButton(credits, text="khalidelmerrah", font=("Segoe UI", 11),
                       fg_color="transparent", hover_color="#2a2a2a", text_color="#4a9eff",
                       anchor="w", height=24, corner_radius=3,
                       command=lambda: webbrowser.open("https://github.com/khalidelmerrah/Windows-App-Updater")).pack(anchor="w")
        # Donate
        ctk.CTkButton(frame, text="Donate to Original Author", font=("Segoe UI", 11),
                       fg_color="#c8a02a", hover_color="#a88520", text_color="#1a1a1a",
                       height=32, corner_radius=4, width=220,
                       command=lambda: webbrowser.open(DONATE_PAGE)).pack(pady=(8, 12))
        # Changelog
        ctk.CTkLabel(frame, text="Changelog", font=("Segoe UI", 12, "bold")).pack(anchor="w", padx=12, pady=(0, 4))
        cl = ctk.CTkTextbox(frame, height=130, font=("Consolas", 9), corner_radius=3)
        cl.pack(fill="x", padx=12, pady=(0, 8))
        cl.insert("1.0", """v2.4.0 - Modern UI
  - customtkinter with Windows 11 dark/light theme
  - Subtle buttons, dark treeview/log, themed progress bars
  - About dialog with credits and changelog

v2.3.0 - Feature Release
  - Search/filter, column sorting, keyboard shortcuts
  - Dark mode, exclude list, Settings dialog, portable config
  - Update history, Retry Failed, scheduled auto-check
  - Windows notifications, app info, export/import, restore point

v2.2.2 - Security Hardening
  - Batch script injection prevention
  - Symlink attack protection, DLL hijacking fix
  - UAC parameter injection fix, URL validation
  - Log cap, thread safety, window maximize fix
""")
        cl.configure(state="disabled")
        # Buttons row
        btn_frame = ctk.CTkFrame(frame, fg_color="transparent")
        btn_frame.pack(fill="x", pady=(4, 0), padx=12)
